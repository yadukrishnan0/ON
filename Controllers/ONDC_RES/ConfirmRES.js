const { isoTimestamp } = require("../../config/unique");
...
const ConfirmRES = async (req, res) => {
  ...
  try {
    ...
    if (!selectObj) {
      return res.status(400).send({ Nack: { message: "No item selected for the transaction." } });
    }
    ...
    if (!initObj) {
      return res.status(400).send({ Nack: { message: "No initialization object found for the selection." } });
    }

    const confirmObj = await Confirm.findOne({initialization:initObj._id});

    if (!confirmObj) {
      return res.status(400).send({ Nack: { message: "No confirmation object found for the initialization." } });
    }
